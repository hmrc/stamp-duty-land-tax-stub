...
@Singleton()
class ManageTransactionController @Inject()(
                                       cc: ControllerComponents,
                                       override val executionContext: ExecutionContext
                                     ) extends BackendController(cc) with StubResource  with Logging{


  def updateTransaction(): Action[JsValue] = Action.async(parse.json) { implicit request =>
    request.body.validate[UpdateTransactionRequest].fold(
      invalid =>
        logger.error(s"[ManageTransactionController][updateTransaction]: Failed to validate payload, errors: $invalid")
