...
@Singleton()
class ManageTaxCalculationController @Inject()(
                                                cc: ControllerComponents,
                                                override val executionContext: ExecutionContext
                                              ) extends BackendController(cc) with StubResource with Logging {

  def updateTaxCalculation(): Action[JsValue] = Action.async(parse.json) { implicit request =>
    request.body.validate[UpdateTaxCalculationRequest].fold(
      invalid =>
        logger.error(s"[ManageTaxCalculationController][updateTaxCalculation]: Failed to validate payload, errors: $invalid")
      ...
      response => {
        ...
        response.returnResourceRef match {
          case "errorUpdatingTaxCalculation" =>
