...
class VendorReturnControllerSpec
  extends AnyWordSpec
  ...
  "GET /" should:
    "return 404 when no return id is found" in:
      val result = testController.vendorReturnDetails(None)(fakeRequest)
      status(result) shouldBe Status.NOT_FOUND

    "return 404 when invalid return id is sent" in :
      val result = testController.vendorReturnDetails(Some("55555"))(fakeRequest)
      status(result) shouldBe Status.NOT_FOUND

    "return 200 when a valid return id has been found" in :
      val result = testController.vendorReturnDetails(Some("123456"))(fakeRequest)
